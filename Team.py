...
class Team:
    ...
    def get_usage_sum(self, list_of_movesets):
        list_of_pokemon = list(set([mon.pokemon.unique_name for mon in list_of_movesets]))
        return sum([Dialgarithm.usage_dict[mon] for mon in list_of_pokemon])
    ...
    def set_ssp(self):
        ...
        for column in team_names:
            ...
            self_loop = self.get_usage_sum(Dialgarithm.counters_dict[column_moveset])
            ...
            normalization_factor = (1 - self_loop) / (sum(transition_mat[column]) - self_loop)
            for row in team_names:
                if row != column:
